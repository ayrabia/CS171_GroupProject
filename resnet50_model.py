...
def build_resnet50(input_shape=(224, 224, 3), num_classes=4):
    ...
    # ResNet50 expects pixel values in [0, 255] range with ImageNet-specific
    # normalization (mean subtraction per channel). The preprocess_input function
    # handles this, but since preprocessing.py already rescales to [0,1], we need
    # to undo that first.
    x = layers.Rescaling(scale=255.0)(inputs)

    # Apply ResNet50's preprocessing (subtracts ImageNet mean per channel)
